...
static CcnetClient *
create_new_client (const char *server_config_dir, const char *conf_dir)
{
    CcnetClient *client;

    client = ccnet_client_new ();
    if (ccnet_client_load_confdir (client, server_config_dir, conf_dir) < 0) {
                         ...
                         size_t fcall_len, size_t *ret_len)
{
    ...
    if (priv->session != NULL) {
        ...
        new_session = create_new_client (session->server_config_dir, session->config_dir);
        if (!new_session) {
            *ret_len = 0;
            return NULL;
        }
